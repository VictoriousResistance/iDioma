...
module.exports = (user1Id, user2Id) => {
  const queryStr = `
                    SELECT ur_1.ur1_id FROM ( 
                      (
                        SELECT user_id AS user1_id, room1_id AS ur1_id FROM users_rooms 
                          WHERE user_id = '${user1Id}' 
                      ) AS ur_1 
                      INNER JOIN 
                      (
                        SELECT user_id AS user2_id, room2_id AS ur2_id FROM users_rooms 
                          WHERE user_id = '${user2Id}' 
                      ) AS ur_2 
                      ON ur_1.room1_id = ur_2.room2_id 
                    )
                    `;
